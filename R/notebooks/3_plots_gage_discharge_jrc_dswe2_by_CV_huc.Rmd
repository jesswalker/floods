...
#####Gages for which r > `r cor_threshold`
...
#####Within-HUC max correlations greater than `r cor_threshold`
...
The _highest_ correlations between monthly imagery data and gage discharge for each dataset in each HUC. 
...
```
####Summary of gage discharge range
...
# Get the max for DSWE by HUC
 ...
 shp %>%
   mutate(HUC8 = as.numeric(HUC8)) %>% 
   left_join(cor.huc.max[cor.huc.max$type == 'dswe', ], c("HUC8" = "huc8.img")) %>% 
   ggplot() + 
     geom_sf(aes(fill = cor)) +
     theme_bw() +
     scale_fill_gradient(limits = c(0,1), low = 'green', high = 'blue') +
...
# ---------------------------------------------------------------------- #
 ...
 shp %>%
   mutate(HUC8 = as.numeric(HUC8)) %>% 
   left_join(cor.huc.max[cor.huc.max$type == 'pekel', ], c("HUC8" = "huc8.img")) %>% 
   ggplot() + 
     geom_sf(aes(fill = cor)) +
     theme_bw() +
     scale_fill_gradient(limits = c(0,1), low = 'green', high = 'blue') +
