...
#   theme(axis.text = element_text(size = 8),  #12
#         axis.title = element_text(size = 10), #14
#         axis.text.x = element_text(vjust = 0.2),
#         axis.title.x = element_blank(), 
#         legend.text = element_text(size = 8),  #12
#         legend.title = element_text(size = 10)) +  #14
#   ylab("% NoData pixels") +
#   scale_color_discrete(name = "Dataset", breaks = c("dswe", "pekel"), labels = c("DSWE", "JRC"))
# 
...
pk_ds_sg.series <-   
  pk_ds_sg.int %>%
  group_by(site_no, huc8.img, huc8.sg, type, cor, p) %>% 
...
pk_ds_sg.int.series <- 
  inner_join(pk_ds_sg.series, pk_ds_sg.int, by = c("site_no", "huc8.img", "huc8.sg", "type", "cor", 'p'))
...
pk_ds_sg.series %>% 
  ...
  group_by(huc8.img) %>% 
  slice(which.max(cor)) %>% 
  distinct(site_no, huc8.img, huc8.sg, type, cor, p) %>% 
  arrange(huc8.img, site_no) %>% 
  kable(col.names = c("Gage", "Imagery HUC", "Gage HUC", "Dataset", 'Corr', 'p')) %>%  
  kable_styling("striped", full_width = FALSE, position = 'left') 
