---
title: "Correlations of gage info with Pekel data"
...
####Workflow  
* Retrieve previously compiled Pekel data (JRC Global Surface Monthly Water History) from all CV HUCs over 2 extents
    + 'Unbuffered' - full HUC
    + 'Buffered' - within 500m of stream extents only
* Clean Pekel data
    + Restrict values to those from images with < 1% NoData pixels
...
* Apply coefficients to stream gage data to estimate missing Pekel data
...
Slightly more buffered than unbuffered data types (262 vs. 206) were associated with higher correlations at each site. 


The highest Pearson correlations between monthly Pekel data and monthly streamgage discharge/height data for each HUC are below. Bolded values are > 0.5.

####Maximum correlation in each HUC
...
# ------------------------------------
# Replace NA Pekel values with interpolated data
# ------------------------------------

pk_sg$water <- ifelse(is.na(pk_sg$water), pk_sg$mean_va*pk_sg$m + pk_sg$b, pk_sg$water)
...
pk_max <-  maxes[order(-maxes$cor), ][1, "pk_type"][[1]]
pk_min <-  maxes[order(-maxes$cor), ][1, "pk_type"][[1]]

huc_max <- maxes[order(-maxes$cor), ][1, "huc8"][[1]]
huc_min <- maxes[order(maxes$cor), ][1, "huc8"][[1]]

sub_max <- subset(pk_sg, site_no == site_max & pk_type == pk_max)
...
Lowest maximum correlation: HUC `r huc_min`, gage `r site_min`  (r = `r format(cor_min, digits = 2)`)
```{r, echo=FALSE, include=FALSE}
sub_min <- subset(pk_sg, site_no == site_min & pk_type == pk_min)

ggplot(subset(sub_min, interp == 'N'), aes(date, water, group = pk_type)) + 
  theme_bw() +
  geom_point(aes(color = pk_type))
...
ggplot(subset(sub_min, interp == 'N'), aes(mean_va, water)) + 
  geom_point() +
...
There are some odd correlations, such as HUC 18040009, site 11359200, which has r = -0.67:

```{r, echo = FALSE}

ggplot(subset(pk_sg, site_no == 11359200 & pk_type == 'unbuffered' & interp == "N"), aes(mean_va, water)) + 
    geom_point() +
...
####Estimating surface water extent based on correlations
What does the maximum correlation look like as far as augmenting associated Pekel data?


```{r, echo=FALSE}
fit_max <- lm(water ~ mean_va, data = sub_max) 

# Get fit coefficients
b_max <- summary(fit_max)$coefficients[1,1]
m_max <- summary(fit_max)$coefficients[2,1]
```
If we replace missing Pekel data water values with the equation

$y = `r b_max` + `r m_max`x_i$  


```{r, echo=FALSE}

ggplot(sub_max, aes(mean_va, water, group = interp, color = interp)) + 
  geom_point() + #(aes(color= pk_type)) +
  geom_smooth(method = "lm", color = "black", formula = y ~ x, size = 0.7) +
  theme_bw() + 
  scale_color_discrete(name = "Interpolated data") +
  xlab(bquote('Discharge ('~ft^3~s^-1*')')) +
  ylab(bquote('Pekel water extent ('~km^2*')'))

```

the augmented Pekel time series yields the following:
```{r, echo = FALSE}

# ---------------------------------------------
# Plot interpolated and non-interpolated data
# ---------------------------------------------

ggplot(sub_max, aes(date, water)) + 
  ...
  ylab(bquote('Pekel water extent ('~km^2*')')) +
         scale_x_date(date_breaks = "2 years",
                    limits = c(as.Date("1992-01-01"), as.Date("2015-01-01")),
                                 date_labels = "%Y") 


```
Note that the interpolation mostly fills in low Pekel data values, but also contributes prominent peaks that were missing from the original Pekel time series.  

Given the high correlation, it's no surprise that the corresponding stream gage monthly data looks very similar:
```{r, echo = FALSE}
ggplot(sub_max, aes(date, mean_va, linetype = parameter_cd)) +
  geom_line(color = "blue") +
    theme_bw() +
    scale_linetype_discrete(name = "Water metric   ") +
  xlab("Date") + 
...
####HUC-specific comparison
Now examine all sites within that one HUC to assess how different values of surface water extent might be derived from different gage correlations. Plots are shown in order from the highest to lowest correlation values. (Buffered and unbuffered plots have different y-axis ranges and cannot be directly compared)

```{r, echo=FALSE}

site_list <- subset(max_sites, huc8 == huc_max)

# order by descending r value
site_list <- site_list[order(-site_list$cor), ]

for (i in 1:nrow(site_list)) {
 row <- site_list[i,]
 data.sub <- subset(pk_sg, site_no == row$site_no & pk_type == row$pk_type)
 
 q <- ggplot(data.sub, aes(mean_va, water, group = interp, color = interp)) + 
      geom_point() + 
      geom_smooth(method = "lm", color = "black", formula = y ~ x, size = 0.7) +
      theme(plot.title = element_text(hjust = 0.5)) +
      ggtitle(paste0(" Site ", row$site_no, "; r = ", format(row$cor, digits =2), "; ", row$pk_type)) +
      theme_bw() + 
      scale_color_discrete(name = "Interpolated data") +
      xlab(bquote('Discharge ('~ft^3~s^-1*')')) +
      ylab(bquote('Pekel water extent ('~km^2*')'))
  ...
  p <- ggplot(data.sub, aes(date, water)) +
       geom_point(aes(color = interp)) +
       geom_line() +
       theme_bw() +
       scale_color_discrete(name = "Interpolated data") +
...
![](D:/projects/floods/graphics/HUC18040002_gages_sm.png?display=inline-block)


####Notes

*  Discharge is the most typical reported gage metric
*  Even gages on the same river (11274550; 11274000) with respectively high correlations result in different estimates of surface water extent
*  Lower correlation gages are on smaller order streams
*  No noise removal was performed on the gage data. Can there be spurious gage values?
*  All correlations assume a linear fit, though more complex relationships may exist
*  Buffered vs. unbuffered data
    + Unbuffered allows for the calculation of surface water across entire HUC 
    + Buffered permits narrower focus on streams (currently undifferentiated)
* What about extending the comparison of gage values to surface water in any given 
HUC--any likely connection?
