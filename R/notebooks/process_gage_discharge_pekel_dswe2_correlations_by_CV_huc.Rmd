...
# - fewer than 5 years of overlapping data exist
sg <- sg %>% 
    group_by(site_no) %>%
...
```{r, echo = FALSE, message=FALSE, include=FALSE}
# ---------------------------------------------------------------------- #
#### Calculate correlation coefficients for imagery and gages in same HUC  ####
# ---------------------------------------------------------------------- #
...
```{r, echo = FALSE, message=FALSE, include=FALSE}
# ---------------------------------------------------------------------- #
#### Calculate correlation coefficients for imagery and gages in all HUCs  ####
# ---------------------------------------------------------------------- #

# Need to combine the datasets in a different way 


# Select the HUC imagery and the corresponding best gage
cor.all.sites.sub <- cor.all.sites %>% 
                     select(huc8.x, site_no) %>% 
                     rename(huc8.img = huc8.x)

# Add in the sg info
sg.mod <- left_join(sg, cor.all.sites.sub, by = c('site_no'))
...
pk_ds_sg.mod <- inner_join(pk_ds, sg.mod, by = c("date" = "date", "huc8" = "huc8.img"))

# Cleanup 
pk_ds_sg.mod <- pk_ds_sg.mod %>%
       mutate(huc8 = factor(huc8), type = factor(type), site_no = factor(site_no), 
              parameter_cd = factor(parameter_cd))

pk_ds_sg.mod$interp <- ifelse(is.na(pk_ds_sg.mod$water), 'Y', 'N')
pk_ds_sg.mod$interp <- as.factor(pk_ds_sg.mod$interp)
...
pk_ds_sg.mod.preinterp <- pk_ds_sg.mod
```
...
pk_ds_sg.mod$water <- ifelse(is.na(pk_ds_sg.mod$water), pk_ds_sg.mod$mean_va * pk_ds_sg.mod$m + pk_ds_sg.mod$b, pk_ds_sg.mod$water)

```

```{r, echo = FALSE}

# Combine pk_ds_sg.mod, pk_ds_sg

pk_ds_sg.mod <- pk_ds_sg.mod %>% select(-huc8.sg)

pk_ds_sg.mod$file <- 'mod'
pk_ds_sg$file <- 'orig'

test <- rbind(pk_ds_sg, pk_ds_sg.mod)

test$file <-  as.factor(test$file)
