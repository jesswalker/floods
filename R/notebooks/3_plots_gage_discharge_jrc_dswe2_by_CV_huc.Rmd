...
file.in <- file.path(path.data, "data", paste0("CV_dswe_jrc_gage_", nodata_threshold, "pct_nodata_correlations_pearson.RData"))
...
####Summary of results for correlations greater than threshold `r cor_threshold`
...
####Range of maximum correlations over all HUCs

```{r, echo = FALSE}
# ---------------------------------------------------------------------- #
# Correlation range for MAX Pekel and DSWE vs. stream gage ####
# ---------------------------------------------------------------------- #
...
####Maximum within-gage correlations for DSWE and JRC
...
####Where DSWE or JRC has a better correlation
...
super_hucs <- unique(as.data.frame(tmp[tmp$n_site ==5, ])['huc8.img'])
...
# Plot super gages
ggplot() + 
  geom_sf(data = t, fill = 'red', color = 'gray50', lwd = 0.4)  +
     geom_sf(data = shp_4326, fill = NA, color = 'gray50', lwd = 0.4) +
  #   geom_sf(data = subset(tmp_4326, n_site == 5), aes(fill = as.factor(n_site))) +
     geom_point(data = subset(site_coords, n_site > 4), aes(x = lon, y = lat), color = "black", shape = 18, size = 6) + 
  theme_bw()
...
This is all gages within each HUC; no maxes.  Gage discharge and "highest confidence" water extent generally have the highest correlations. 

```{r, echo = FALSE}
 
ds_sg.cor %>% 
      ungroup(huc8.sg) %>% 
      select(huc8.img, site_no, parameter_cd, cor1, cor2, cor12, cor123, cor1234, n) %>% #, -huc8.sg)
      kable(col.names = c("HUC", "Site", "Gage type", "r (w1)", "r (w2)", "r (w1+w2)", "r (w1+w2+w3)", "r (w1+w2+w3+w4)", "n"), digits = 3) %>%  
      kable_styling("striped", full_width = FALSE, position = 'left') %>%
      row_spec(c(which(ds_sg.cor$cor1 > cor_threshold)), bold = T)
