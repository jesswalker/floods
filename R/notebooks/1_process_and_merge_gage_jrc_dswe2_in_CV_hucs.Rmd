...
####Workflow 
  ...
  - Retrieve JRC and DSWE monthly composites within all Central Valley HUCs from Google Drive 
    - Process JRC and DSWE files to convert values, standardize names, etc. 
    - In scenes with > **`r nodata_threshold`%** No Data, zero out water values pending interpolation
  - Retrieve metadata from NWIS for all gages in Central Valley
  - Assess gages records for length of data, data type
    - Gage record has to include at least 5 years of data after 1984
    - Discharge or gage height
  - Retrieve stream gage data from NWIS
  - Combine stream gage, JRC, and DSWE data for all HUCs
  
####Output
  - Stream gage, JRC, and DSWE file written to **`r file.path(path.out, 'data', file.out)`**
...
# This allows us to narrow the list of desired gages based on the data available in each
