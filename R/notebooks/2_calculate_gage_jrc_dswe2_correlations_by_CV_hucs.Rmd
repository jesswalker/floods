...
cor_type <- 'spearman' #pearson' #'kendall' # 'pearson' 'spearman' 
...
pk_ds_sg.cor.all <- pk_ds_sg %>% 
  ...
  mutate(cor = cor.test(water, mean_va, use = "pairwise.complete.obs", method = cor_type),
            n = sum(!is.na(water))) 
...
#### each dataset, meaning the gages can be different.
...
pk_ds.cor <- pk_ds.sub %>% 
    ...
    summarize(cor = cor(water_pk, water_ds, use = "pairwise.complete.obs", method = cor_type), 
