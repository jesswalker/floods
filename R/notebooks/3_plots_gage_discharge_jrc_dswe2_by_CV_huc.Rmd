...
#If the table rather than the plot is displayed, use olded values are > 0.5.

maxes <- cor.huc.max %>%
    select(huc8.img, site_no, parameter_cd, type, cor, n, -c(huc8.sg)) %>% 
...
p <- ggplot(maxes, aes(x = reorder(huc8.img, -cor), y = cor, fill = type)) + 
...
t <-  cor.all.max %>% 
      ...
      slice(which.max(cor)) %>% 
#%>% 
  select(huc8.img, site_no, huc8.sg, type, cor, same_huc, n_site, -c(parameter_cd, n, b, m, interp, water2, water3, water4)) %>% 
...
ds_sg.cor %>%  
 #   select(huc8.img, site_no, parameter_cd, cor1, cor12, cor123, cor124, n) %>% 
