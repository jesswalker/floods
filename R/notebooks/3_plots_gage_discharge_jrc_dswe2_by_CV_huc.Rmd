...
file.in <- file.path(path.data, "data", paste0("CV_dswe_jrc_gage_", nodata_threshold, "pct_nodata_correlations.RData"))
...
#If the table rather than the plot is displayed, use bolded values are > 0.5.
 ...
 cor.huc.max %>%
     ...
     row_spec(c(which(cor.huc.max$cor > 0.5)), bold = T) 
```

####Summary of results for correlations greater than 0.7
...
cor.huc.max %>% 
 ...
 kable(col.names = c("Type", "n", "Mean r", "Median r", "% of HUCs"), digits = 3) %>%  
     kable_styling("striped", full_width = FALSE)  
```
...
# 
# huc_max <- maxes[order(-maxes$cor), ][1, "huc8.img"][[1]]
# 
# huc_min <- maxes[order(maxes$cor), ][1, "huc8.img"][[1]]
# 
# sub_max <- subset(pk_ds_sg.int, site_no == site_max & type == pk_max)
...
# Breaking it up like this is necessary b/c kable can't handle the fact that cor.all.max has changed ('select')
t %>% 
  kable(col.names = c("HUC - imagery", "Gage w best correlation", "HUC - gage", "Data type", "r", 'Same HUC?','# times gage is best'), digits = 3) %>%
  kable_styling("striped", full_width = FALSE) %>%
  row_spec(c(which(t$cor > 0.5)), bold = T) %>%
...
p <- ggplot(tmp) + 
     geom_sf(aes(fill = as.factor(n_site))) +
...
ds_sg.cor %>% 
      ungroup(huc8.sg) %>% 
      select(huc8.img, site_no, parameter_cd, cor1, cor12, cor123, cor124, n) %>% #, -huc8.sg)
      kable(col.names = c("HUC", "Site", "Gage type", "r (w1)", "r (w1+w2)", "r (w1+w2+w3)", "r (w1+w2+w4)", "n"), digits = 3) %>%  
      kable_styling("striped", full_width = FALSE)%>%
      row_spec(c(which(ds_sg.cor$cor1 > 0.5)), bold = T)
