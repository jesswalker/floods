...
# pk_ds_sg.int - file of all interpolated values for imagery HUC = sg HUC, and ONLY the values corresponding to the max correlation for imagery HUC != sg HUC
...
####Identify long-term time series and missing months of data
...
pk_ds_sg.long <-   
  ...
  summarize(n = n()) %>% 
  filter(n > 300 & cor > 0.7)

pk_ds_sg.int.long <- 
  inner_join(pk_ds_sg.long, pk_ds_sg.int, by = c("site_no", "huc8.img", "huc8.sg", "type", "cor"))

write.csv(pk_ds_sg.int.long, file = "E:/projects/place/output/tables/pk_ds_sg_gt30years_and_corgt0pt7.csv", row.names = FALSE)

```

Which internal-match HUCs had 30+ years of data and a correlation basis > 0.7?
```{r, echo = FALSE}
...
pk_ds_sg.int.long %>% 
  ...
  kable(col.names = c("Gage", "Imagery HUC", "Gage HUC", "Dataset", 'Corr')) %>%  
  kable_styling("striped", full_width = FALSE, position = 'left') %>% 
    scroll_box(height = '400px', width = '400px')
...
Which external-match HUCs had 30+ years of data and a correlation basis > 0.7?
```{r, echo = FALSE}
...
```
How man distinct HUCs had 30+ years of data and a correlation basis > 0.7?
```{r, echo = FALSE}
