...
data_url = "1SyF1ds8XuxxGW9-UbEbBfhPE65bg_FJv"
#data_url = "USGS_streamgages_Central_Valley.geojson"

#gages <- geojsonio::geojson_read(sprintf("https://docs.google.com/uc?id=%s&export=download", data_url), #what = "sp")

test <- "c:/temp/gages.geojson"


download.file(sprintf("https://docs.google.com/uc?id=%s&export=download", data_url), test)    
data.json <- geojsonio::geojson_read(test, what = "sp")     

plot(data.json)
 
map.image <- ggmap(get_googlemap(c(lon = -119.636181, lat = 38.990928), scale = 1, zoom = 8), extent = "normal")        


data.df <- as.data.frame(data.json)
names(data.df)[10:11] <- c("lon", "lat")

print(map.image + geom_point())




                                 
#gages <- geojsonio::geojson_read(sprintf("d:/projects/floods/gis/data/%s", data_url), what = "sp")
...
# data_url = "1eMwZUVX5SvbeXUqlV54ijVJs7fL0zr1m"
# 
# hucs = geojsonio::geojson_read(sprintf("https://docs.google.com/uc?id=%s&export=download", data_url), what = "sp")
# 
# leaflet(hucs) %>%
#   addTiles() %>%
