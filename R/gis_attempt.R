...
#hucs <- shapefile(file.path(path.in, huc8_name))
hucs <- st_read(file.path(path.in, huc8_name))
st_crs(hucs)  

# make sure hucs projection is UTM
#hucs <- spTransform(huc8s, crs("+init=epsg:32610"))

# Load river extents as shapefiles                            
stream_name <- "rg_ex_monthly_water_2011_02_clip_dissolve_prox.shp"
stream <-  read_sf(file.path(path.in, "test/utm", "rg_ex_monthly_water_2011_02_clip_dissolve_prox.shp"))
st_crs(stream) <- "+proj=utm +zone=10 +datum=WGS84 +units=m +no_defs +ellps=WGS84 +towgs84=0,0,0"
...
streams <-  rgeos::gSimplify(stream, tol = 0.00001)

streams <-  gSimplify(stream, tol = 0.00001)
int = as_tibble(st_intersection(stream, hucs))
