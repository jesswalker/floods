...
####Workflow  
* Retrieve Pekel data from all 50 CV HUCs over 2 extents
    + 'Unbuffered' - full HUC
    + 'Buffered' - within 500m of stream extents
* Clean Pekel data
    + Restrict values to those from images with < 1% NoData pixels
* Retrieve stage height and discharge data from all CV stream gages
* Perform Pearson correlation on datasets
* Retrieve linear fit coefficients
* Apply coefficients to stream gage data to calculate missing Pekel data
* Assemble plots
...
#pk_btest <- pk_b %>% 
#        group_by(huc8) %>%
#        filter(length(water) > sum(is.na(water)))
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, warn.conflicts = FALSE}
# Remove data in each HUC that are < 1.5SD from mean

#pk <- pk %>%
#  group_by(pk_type, huc8) %>%
#  filter(water > (mean(water, na.rm = TRUE) - 1.5 *sd(water, na.rm = T)))
...
# We'll narrow the list of desired gages based on the data available in each
...
# Remove gages in which all values are the same
sg <- sg %>% 
    group_by(site_no) %>%
    filter(length(unique(mean_va)) > 1)

#  Remove rows with gage values < 0
sg <- sg[sg$mean_va >= 0, ]
...
```
The highest Pearson correlations between monthly Pekel data and monthly streamgage discharge data for each HUC are below. Bold values are > 0.5.
...
```{r, echo=FALSE}
# ------------------------------------
# Replace NAs with interpolated data
# ------------------------------------
...
maxes %>%
    kable(col.names = c("HUC", "Gage", "Pekel data", "Parameter", "r")) %>%  
...
Highest correlation: gage `r site_max`
...
Lowest correlation: gage `r site_min[[1]]`
```{r, echo=FALSE, include=FALSE}
sub_min <- subset(pk_sg, site_no == site_min & pk_type == pk_min)

ggplot(sub_min, aes(date, water, group = pk_type)) + 
  theme_bw() +
  geom_point(aes(color = pk_type))
...
```
What does the max look like as far as augmenting associated Pekel data?

Max correlation:

```{r, echo=FALSE}
fit_max <- lm(water ~ mean_va, data = subset(pk_sg, site_no == site_max & pk_type == pk_max & interp == 'N')) 
...
df <- subset(pk_sg, site_no == site_max & pk_type == pk_max)

ggplot(df, aes(mean_va, water, group = interp, color = interp)) + 
  geom_point() + #(aes(color= pk_type)) +
...
```
Corresponding stream gage monthly data:
```{r, echo = FALSE}
ggplot(subset(sg, site_no == site_max), aes(date, mean_va/2000, linetype = parameter_cd)) +
...
```
You can see that the interpolation mostly fills in low Pekel data values, but also contributes prominent peaks that were missing from the original Pekel time series.
