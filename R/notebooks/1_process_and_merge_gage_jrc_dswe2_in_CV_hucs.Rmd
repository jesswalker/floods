...
gage_info <- as.data.frame(g[, c('site_no', 'huc_cd', 'dec_lat_va', 'dec_long_va')])
gage_info <- gage_info[-1,]
gage_info <- unique(gage_info[c("site_no", "huc_cd",  'dec_lat_va', 'dec_long_va')])
gage_info <- gage_info %>% 
...
# Restrict to discharge info only
sg <- subset(sg, parameter_cd == "00060")

# Replace parameter info
sg[sg$parameter_cd == "00060", "parameter_cd"] <- "discharge"

# Attribute sites with HUC#
sg <- merge(sg, gage_info, by = "site_no")
