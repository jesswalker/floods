...
The _highest_ Pearson correlations between monthly imagery data and monthly streamgage discharge/height data for each respective imagery set in each HUC are below. Bolded values are > 0.5.
...
for (huc in unique(maxes$huc8))
  
  
  
  - get the site_no for dswe
  - get site_no for pekel


# all imagery data - recorded and interpolated
  df.all.dswe <- subset(pk_ds_sg, huc8 == '18020123' & site_no == 11406820 & type == 'dswe')
  df.all.pekel <- subset(pk_ds_sg, huc8 == '18020123' & site_no == 11406820 & type == 'pekel')

  pk_ds_sg.all <- rbind(df.all.dswe, df.all.pekel)
...
huc.name <- paste0("HUC ", df.all$huc8[1])

  plot_interpolated_time_series <- function(df){
...
huc.name <- paste0("HUC ", pk_ds_sg.all$huc8[1])
...
p <- ggplot(pk_ds_sg.all, aes(date, water, group = type)) + 
      ...
      geom_point(data = pk_ds_sg.all, aes(date, water, color = interp)) +
